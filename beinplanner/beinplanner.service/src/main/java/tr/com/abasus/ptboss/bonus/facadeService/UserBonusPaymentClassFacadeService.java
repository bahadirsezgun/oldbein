...
@Service
@Scope("prototype")
public class UserBonusPaymentClassFacadeService implements UserBonusPaymentFacadeService {
	...
	public boolean isLockBonusPayedClasses(long schtId) {
		...
		List<UserBonusPaymentClass> userBonusPaymentClasss=bonusClassService.findUserOfMonth(schtStaffId, month, year);
		////System.out.println("BONUS LOCK KONTROL START MONTH:"+month+"  YEAR:"+year+" STAFF ID:" +schtStaffId);
		for (UserBonusPaymentClass userBonusPaymentClass : userBonusPaymentClasss) {
			Date bonEndDate=userBonusPaymentClass.getBonEndDate();
