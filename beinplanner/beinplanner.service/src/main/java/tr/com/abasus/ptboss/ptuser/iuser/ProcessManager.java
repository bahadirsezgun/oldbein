...
@Component(value="processManager")
@Scope("prototype")
public class ProcessManager  implements ProcessInterface {


	@Autowired
	@Qualifier(value="processSchedulerStaff")
	...
	public int getUserCompletePercent(User user) {
		return processUserDao.getUserCompletePercentToManager(user);
	}

	@Override
	public List<User> findAll(int firmId) {
		return processUserDao.findAllToManager(firmId);
	}

	@Override
	public List<User> findAllInChain(int firmId) {
		
		List<User> managers=processUserDao.findAllToManager(firmId);
		List<User> chainUsers=processInterface.findAllInChain(firmId);
		if(managers.size()>0)
			chainUsers.addAll(managers);
		return chainUsers;
	}
	
	@Override
	public List<User> findByUserNameAndSurnameInChain(String userName, String userSurname, int firmId) {
		List<User> managers=processUserDao.findByNameAndSurnameToManager(userName, userSurname,firmId);
	...
	public MenuTbl getMenuDashBoard() {
		return menuService.findDashBoardByUserType(UserTypes.USER_TYPE_MANAGER_INT);
	}
	
	@Override
	public List<MenuTbl> getMenuSide() {
		List<MenuTbl> menuTbls=menuService.findUpperMenuByUserType(UserTypes.USER_TYPE_MANAGER_INT);
		for (MenuTbl menuTbl : menuTbls) {
			List<MenuLevel2> menuLevel2s=menuService.findLevel2MenuByUserType(UserTypes.USER_TYPE_MANAGER_INT, menuTbl.getMenuId());
	...
	public List<MenuTbl> getMenuTop() {
		List<MenuTbl> menuTopTbls=menuService.findTopMenuByUserType(UserTypes.USER_TYPE_MANAGER_INT);
		return menuTopTbls;
	}

	@Override
	public List<User> findByNameAndSurname(String userName, String userSurname,int firmId) {
		return processUserDao.findByNameAndSurnameToManager(userName, userSurname,firmId);
	}
